      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="p-4 border-b border-slate-800 flex items-center justify-between bg-slate-900/50">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-slate-200">Images ({images.length})</h2>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="bg-slate-800 border border-slate-700 rounded-lg pl-9 pr-3 py-1.5 text-sm text-slate-200 focus:outline-none focus:border-blue-500 w-48"
              />
            </div>
            ...
            <div className="grid grid-cols-1 gap-3">
              {filteredImages.map(image => (
                <div
                  ...
                  className={`group relative bg-slate-900/50 border rounded-xl p-4 transition-all cursor-pointer ${selectedImageId === image.id ? 'border-blue-500 bg-blue-900/10' : 'border-slate-800 hover:border-slate-700'}`}
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded bg-slate-800 flex items-center justify-center text-yellow-500">
                        <Layers size={20} />
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          <h3 className="font-bold text-slate-200">{image.repository}</h3>
                          <span className="px-1.5 py-0.5 bg-slate-800 text-slate-400 text-xs rounded font-mono">:{image.tag}</span>
                    ...
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        className="p-2 hover:bg-blue-900/30 text-slate-400 hover:text-blue-400 rounded transition-colors"
                        title="Run"
                      >
                      ...
                      <button
                        ...
                        className="p-2 hover:bg-slate-800 text-slate-400 hover:text-slate-200 rounded transition-colors"
                        title="Push"
                      >
                        <Upload size={16} />
                      </button>
                      ...
                      <button
                        ...
                        className={`p-2 hover:bg-slate-800 rounded transition-colors ${image.protected ? 'text-green-500' : 'text-slate-400 hover:text-green-400'}`}
                  ...
                  {selectedImageId === image.id && (
                    <div className="mt-4 pt-4 border-t border-slate-800 grid grid-cols-2 gap-4 text-xs animate-in fade-in slide-in-from-top-2">
                      <div>
                        ...
                        <div className="space-y-1 max-h-40 overflow-y-auto pr-2">
                          {image.layers?.map((layer, idx) => (
                            <div key={idx} className="flex gap-2 p-1.5 bg-slate-800/30 rounded border border-slate-800/50">
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          <div>
                            ...
                            <code className="bg-slate-800 px-1.5 py-0.5 rounded text-green-300 font-mono block truncate">
                          ...
                          <div>
                            ...
                            <div className="flex flex-col gap-1">
                              {image.config?.env?.slice(0, 3).map((e, i) => (
                                <code key={i} className="bg-slate-800 px-1.5 py-0.5 rounded text-blue-300 font-mono truncate">{e}</code>
                              ))}
