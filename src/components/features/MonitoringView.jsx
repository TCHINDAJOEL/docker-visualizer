...
const MonitoringView = ({ containers, timeline }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-slate-950 text-slate-200">
            {/* Header / Tabs */}
            <div className="flex items-center border-b border-slate-800 bg-slate-900/50 px-6 pt-4">
                <button
                    onClick={() => setActiveTab('dashboard')}
                    className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'dashboard' ? 'border-blue-500 text-blue-400' : 'border-transparent text-slate-500 hover:text-slate-300'}`}
                >
                    <Activity size={16} /> Dashboard
                </button>
                <button
                    onClick={() => setActiveTab('logs')}
                    className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'logs' ? 'border-blue-500 text-blue-400' : 'border-transparent text-slate-500 hover:text-slate-300'}`}
                >
                    <Terminal size={16} /> Consolidated Logs
                </button>
                <button
                    onClick={() => setActiveTab('events')}
                    className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'events' ? 'border-blue-500 text-blue-400' : 'border-transparent text-slate-500 hover:text-slate-300'}`}
                >
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 h-full overflow-y-auto">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl">
                                <div className="text-slate-500 text-xs uppercase font-bold mb-1">Total CPU Load</div>
                                <div className="text-2xl font-mono text-blue-400">{stats.totalCpu}%</div>
                                <div className="text-xs text-slate-600 mt-1">Across {stats.runningCount} containers</div>
                            </div>
                            <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl">
                                <div className="text-slate-500 text-xs uppercase font-bold mb-1">Total Memory</div>
                                <div className="text-2xl font-mono text-purple-400">{stats.totalMem} MB</div>
                                <div className="text-xs text-slate-600 mt-1">Reserved</div>
                            </div>
                            <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl">
                                <div className="text-slate-500 text-xs uppercase font-bold mb-1">Health Status</div>
                            ...
                            <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl">
                                <div className="text-slate-500 text-xs uppercase font-bold mb-1">Events (24h)</div>
                                <div className="text-2xl font-mono text-slate-300">{timeline.length}</div>
                                <div className="text-xs text-slate-600 mt-1">System activities</div>
                            </div>
                        </div>

                        {/* Container Health List */}
                        <div className="bg-slate-900/50 border border-slate-800 rounded-xl overflow-hidden">
                            <div className="px-4 py-3 border-b border-slate-800 bg-slate-900 font-bold text-sm text-slate-300">
                                Container Health & Resources
                            </div>
                            <div className="divide-y divide-slate-800">
                                {containers.filter(c => c.status === 'running').map(container => (
                                    <div key={container.id} className="px-4 py-3 flex items-center justify-between hover:bg-slate-800/30 transition-colors">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <div className="text-xs text-slate-500 font-mono">{container.id.substring(0, 12)}</div>
                                            </div>
                                        </div>
                                ...
                                {containers.filter(c => c.status === 'running').length === 0 && (
                                    <div className="p-8 text-center text-slate-500 italic">No running containers to monitor.</div>
                                )}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col h-full animate-in fade-in slide-in-from-bottom-2">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <button
                                ...
                                className={`p-2 rounded-lg border ${isPaused ? 'bg-yellow-500/10 border-yellow-500/50 text-yellow-500' : 'bg-slate-800 border-slate-700 text-slate-400 hover:text-white'}`}
                        ...
                        <div className="flex-1 bg-black rounded-xl border border-slate-800 p-4 font-mono text-xs overflow-y-auto">
                            {filteredLogs.length === 0 ? (
                                <div className="text-slate-600 text-center mt-10">No logs found matching filters.</div>
                            ) : (
                                filteredLogs.map((log, idx) => (
                                    <div key={log.id || idx} className="mb-1 flex gap-3 hover:bg-slate-900/50 p-0.5 rounded">
                                        <span className="text-slate-600 shrink-0 w-32 truncate" title={log.timestamp}>
                ...
                {activeTab === 'events' && (
                    ...
                    ${event.type === 'error' ? 'bg-red-500' :
                        ...
                        ${event.type === 'error' ? 'bg-red-500/10 text-red-400' :
                                        ...
                                        <p className="text-sm text-slate-300">{event.content || event.message}</p>
                                    </div>
                                </div>
                            ))}
                            {timeline.length === 0 && (
                                <div className="pl-6 text-slate-500 italic">No events recorded yet.</div>
                            )}
                        </div>
                    </div>
                )}
