...
const StacksView = ({ stacks, onDeploy, onStop, onRemove, onCreate }) => {
    ...
    return (
        <div className="flex h-full bg-transparent text-zinc-200">
            {/* Sidebar List */}
                    ...
                    {stacks.map(stack => (
                        <button
                            ...
                            className={`w-full text-left p-3 rounded-lg border transition-all flex justify-between items-center group
                                ${selectedStackId === stack.id ? 'bg-blue-900/20 border-blue-500/50' : 'bg-white/5 border-white/10 hover:border-white/20'}
                            `}
                        >
                            <div>
                                <div className="font-medium text-sm text-zinc-200">{stack.name}</div>
                        ...
                        <div className="p-4 border-b border-white/10 bg-white/5 flex justify-between items-center">
                            <div>
                                <h1 className="text-xl font-bold text-white flex items-center gap-3">
                                    {selectedStack.name}
                                    <span className={`text-xs px-2 py-0.5 rounded-full border ${selectedStack.status === 'active' ? 'bg-green-500/10 text-green-400 border-green-500/30' : 'bg-white/5 text-zinc-400 border-white/10'}`}>
                            ...
                            <div className="flex gap-2">
                                {selectedStack.status !== 'active' ? (
                                    ...
                                    <button onClick={() => onStop(selectedStack.id)} className="px-3 py-1.5 bg-white/10 hover:bg-white/20 text-zinc-200 rounded text-sm font-medium flex items-center gap-2">
                                        <Square size={16} /> Stop
                                    </button>
                                )}
                                <button onClick={() => handleEdit(selectedStack)} className="px-3 py-1.5 bg-white/10 hover:bg-white/20 text-zinc-300 rounded text-sm font-medium flex items-center gap-2">
                                    <Edit size={16} /> Edit
                                </button>
                        ...
                        <div className="flex-1 overflow-hidden flex">
                            {isEditing ? (
                                <div className="flex-1 flex flex-col">
                                    <div className="bg-black/40 border-b border-white/10 p-2 flex justify-between items-center">
                                        <span className="text-xs text-zinc-500 font-mono ml-2">docker-compose.yml</span>
                                        <div className="flex gap-2">
                                            <button onClick={() => setIsEditing(false)} className="px-3 py-1 text-xs text-zinc-400 hover:text-white">Cancel</button>
                                    ...
                                    <textarea
                                        ...
                                        className="flex-1 bg-black/80 text-zinc-300 font-mono text-xs p-4 resize-none focus:outline-none"
                                        spellCheck="false"
                                    />
                                </div>
                            ) : (
                                <div className="flex-1 p-6 overflow-y-auto">
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                        ...
                                        <div className="space-y-4">
                                            <h3 className="text-sm font-bold text-zinc-400 uppercase tracking-wider">Services</h3>
                                            {Object.values(selectedStack.services || {}).map((service, idx) => (
                                                <div key={idx} className="bg-black/40 border border-white/10 rounded-lg p-4">
                                                    <div className="flex justify-between items-start mb-2">
                                                        ...
                                                        <div className="text-xs bg-white/5 px-2 py-0.5 rounded text-zinc-400">{service.image}</div>
                                                    </div>
                                                    <div className="space-y-1 text-xs text-zinc-500 font-mono">
                                                        {service.ports?.length > 0 && (
                                                            <div className="flex gap-2">
                                                                <span className="w-16">Ports:</span>
                                                                <span className="text-zinc-300">{service.ports.join(', ')}</span>
                                                            </div>
                                                        )}
                                                        {service.environment?.length > 0 && (
                                                            <div className="flex gap-2">
                                                                <span className="w-16">Env:</span>
                                                                <span className="text-zinc-300">{service.environment.length} vars</span>
                                                            </div>
                                                        )}
                                                        {service.volumes?.length > 0 && (
                                                            <div className="flex gap-2">
                                                                <span className="w-16">Volumes:</span>
                                                                <span className="text-zinc-300">{service.volumes.join(', ')}</span>
                                                            </div>
                                                        )}
                                                    </div>
                                                </div>
                                        ...
                                        <div className="glass border border-white/10 rounded-xl p-6 flex flex-col items-center justify-center text-center min-h-[300px]">
                                            <Layers size={48} className="text-zinc-700 mb-4" />
                                            <h3 className="text-zinc-400 font-medium mb-2">Stack Visualization</h3>
